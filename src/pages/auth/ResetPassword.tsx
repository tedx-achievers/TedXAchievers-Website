...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.email || !formData.code) {
      setValidationError("Email and reset code are required");
      return;
    }
    ...
    try {
      await resetPassword({ email: formData.email, code: formData.code, newPassword: formData.password });
      navigate('/login');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full bg-[url('/home_hero_bg.jpg')] bg-cover bg-center bg-no-repeat relative flex items-center justify-center py-24 px-4 overflow-hidden">
      <SEO title="Set New Password | TEDxAchievers University" description="Set your new password." />
      ...
      >
        ...
        <form className="space-y-5 relative z-10" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">Reset Code</label>
            <input 
              type="text" 
              name="code"
              value={formData.code}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">New Password</label>
          ...
          <button 
            type="submit" 
            disabled={isLoading || !formData.email || formData.code.length < 6 || (formData.password !== formData.confirmPassword && formData.password.length > 0)}
            className={`w-full ${isLoading || !formData.email || formData.code.length < 6 || (formData.password !== formData.confirmPassword && formData.password.length > 0) ? 'bg-[#333] text-gray-400 cursor-not-allowed' : 'bg-[#e62b1e] text-white hover:bg-red-700'} border border-white/5 font-bold py-4 rounded-full uppercase tracking-widest text-sm transition-all mt-6`}
          >
